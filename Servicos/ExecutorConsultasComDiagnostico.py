...
class ExecutorConsultasComDiagnostico:
    ...
    def _testar_conexao_inicial(self):
        try:
            self.logger.info(f"Testando conexão inicial: {self.configuracao}")
...
class ExecutorConsultaUnicaComDiagnostico:
    ...
    def iniciar_execucao(self):
        if not self.executando:
            ...
            self.thread.start()
            self.logger.info(f"Thread {self.thread_id} iniciada para {self.nome_consulta}")
    ...
    def parar_execucao(self):
        if self.executando:
            ...
            self.logger.info(
                f"Thread {self.thread_id} parada - {self.nome_consulta} - Consultas: {self.contador_consultas}")
    ...
    def _executar_loop(self):
        ...
        self.logger.info(f"Thread {self.thread_id} - Iniciando execução de {self.nome_consulta}")

        while self.executando:
            try:
                if conexao.executar_consulta(self.sql_consulta):
                    self.contador_consultas += 1
                    if self.contador_consultas % 50 == 0:
                        self.logger.info(
                            f"Thread {self.thread_id} - {self.nome_consulta}: {self.contador_consultas} consultas")
                else:
        ...
        conexao.desconectar()
        self.logger.info(f"Thread {self.thread_id} - Loop finalizado para {self.nome_consulta}")
