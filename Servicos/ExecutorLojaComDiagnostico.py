...
class ExecutorLojaComDiagnostico:
    ...
    def iniciar_execucao(self):
        if not self.executando:
            ...
            self.thread_gerenciador = threading.Thread(target=self._gerenciar_fila_lojas,
                                                       name=f"GerenciadorDiag-{self.ip_porta}")
    ...
    def _gerenciar_fila_lojas(self):
        self.logger.info(f"Iniciando gerenciamento com diagnóstico para IP {self.ip_porta}")
        ...
        while self.executando:
            try:
                ...
                if contador_ciclos % 30 == 0:  # Log a cada 30 ciclos
                    self.logger.info(
                        f"IP {self.ip_porta} - Ciclo {contador_ciclos} - Ativos: {len(self.executores_ativos)}")
        ...
        self.logger.info(f"Gerenciamento com diagnóstico finalizado para IP {self.ip_porta}")
    ...
    def _iniciar_executor_loja(self, loja):
        if loja.nome_banco not in self.executores_ativos:
            ...
            if stats and stats.get('conexao_ativa'):
                self.logger.info(f"✅ Executor iniciado com sucesso para loja {loja.nome_banco} no IP {self.ip_porta}")
            else:
